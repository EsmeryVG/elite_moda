    ...
    <tr class="producto-row {{ $producto->variantes->count() > 0 ? '' : 'no-expand' }} align-middle"
        ...
        <td style="width: 32px;" class="text-center">
            @if ($producto->variantes->count() > 0)
                <svg class="chevron-icon text-secondary" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"
        ...
        <td class="text-end">
            <a href="{{ route('productos.show', $producto) }}" class="btn btn-outline-info btn-sm" title="Ver detalle">
                <i class="bi bi-eye"></i>
            </a>
            @permiso('productos.gestionar')
                <a href="{{ route('productos.edit', $producto) }}" class="btn btn-outline-warning btn-sm" title="Editar">
                    <i class="bi bi-pencil-square"></i>
                </a>
                @if ($producto->estado)
                    <form action="{{ route('productos.desactivar', $producto) }}" method="POST" class="d-inline-block">
                        @csrf @method('PATCH')
                ...
                @else
                    <form action="{{ route('productos.reactivar', $producto) }}" method="POST" class="d-inline-block">
                        @csrf @method('PATCH')
            ...
            <td colspan="8" class="p-3">
                <div class="variantes-inner p-3 bg-white rounded border shadow-sm mx-4">
                    <div class="d-flex align-items-center mb-2">
                        <span class="fw-semibold text-secondary" style="font-size: 13px;">Desglose de
                            Combinaciones</span>
                    </div>
                    <table class="table table-sm table-hover table-striped align-middle mb-0 w-100"
                                        ...
                                        @else
                                            @foreach ($variante->valores as $valor)
                                                <span class="badge bg-light text-dark border me-1"
                                                    ...
                                                    <strong>{{ $valor->atributo?->nombre }}:</strong>
                                                    {{ $valor->valor }}
                                                </span>
                                            @endforeach
                                        @endif
