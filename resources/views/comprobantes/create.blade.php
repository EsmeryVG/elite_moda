...
@section('page_title', 'Nuevo Comprobante Fiscal')
@section('page_subtitle', 'Registra un nuevo rango de NCF autorizado por la DGII')
                ...
                <form action="{{ route('comprobantes.store') }}" method="POST" id="formComprobante">
                    ...
                    <div class="mb-3">
                        ...
                        <select name="tipo_comprobante"
                            ...
                            <option value="Consumidor Final" {{ old('tipo_comprobante') === 'Consumidor Final' ? 'selected' : '' }}>
                                Consumidor Final (B02)
                            </option>
                            <option value="Crédito Fiscal" {{ old('tipo_comprobante') === 'Crédito Fiscal' ? 'selected' : '' }}>
                                Crédito Fiscal (B01)
                            </option>
                            <option value="Nota de Crédito" {{ old('tipo_comprobante') === 'Nota de Crédito' ? 'selected' : '' }}>
                                Nota de Crédito (B04)
                            </option>
                            <option value="Regímenes Especiales" {{ old('tipo_comprobante') === 'Regímenes Especiales' ? 'selected' : '' }}>
                                Regímenes Especiales (B14)
                            </option>
                            <option value="Gubernamental" {{ old('tipo_comprobante') === 'Gubernamental' ? 'selected' : '' }}>
                                Gubernamental (B15)
                            </option>
                        </select>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" name="prefijo_ncf"
                               ...
                               value="{{ old('prefijo_ncf') }}"
                               placeholder="Ej: B02" maxlength="5">
                        @error('prefijo_ncf')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="row g-3">
                        <div class="col-md-6">
                            ...
                            <input type="number" name="rango_inicio"
                                   ...
                                   value="{{ old('rango_inicio', 1) }}" min="1">
                            @error('rango_inicio')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
            ...
            <div class="card-body p-4">
                ...
                <div class="d-grid gap-2">
                    <button type="submit" form="formComprobante" class="btn btn-primary">
                        <i class="bi bi-check-circle me-1"></i> Registrar comprobante
                    </button>
