/* =====================================================
   Almacenes — JS del módulo
   ===================================================== */

const AlmacenesModule = (function () {
    ...
    let sucursalActual = new URLSearchParams(window.location.search).get('sucursal') ?? '';
    ...
    function cargarTabla(params = {}) {
        ...
        if (params.sucursal) url.searchParams.set('sucursal', params.sucursal);
    ...
    function bindPaginacion() {
        document.querySelectorAll('.ajax-page').forEach(link => {
            link.addEventListener('click', function (e) {
                ...
                cargarTabla({ buscar: buscarActual, estado: estadoActual, sucursal: sucursalActual, page });
            });
        });
    }
    ...
    function bindFiltros() {
        document.querySelectorAll('.em-filtro').forEach(btn => {
            btn.addEventListener('click', function () {
                ...
                cargarTabla({ buscar: buscarActual, estado: estadoActual, sucursal: sucursalActual });
            });
        });
    ...
    function bindBuscador() {
        const input = document.getElementById('buscadorAlmacenes');
        if (!input) return;
...
document.addEventListener('DOMContentLoaded', () => AlmacenesModule.init());