const CategoriasModule = (function () {
    ...
    function bindFiltros() {
        document.querySelectorAll('.em-filtro').forEach(btn => {
            btn.addEventListener('click', function () {
                ...
                buscarActual = document.getElementById('buscadorCategorias')?.value.trim() ?? '';
    ...
    function init() {
        if (!document.getElementById('tablaContainer')) return;
        bindFiltros();
        bindBuscador();
        bindPaginacion();
    }
...
document.addEventListener('DOMContentLoaded', () => CategoriasModule.init());